...
DROP POLICY IF EXISTS "Service role can manage claims" ON public.claims;
...
DROP POLICY IF EXISTS "Service role can manage rate changes" ON public.rate_changes;
...
-- Drop existing policies if they exist (IDEMPOTENTE)
DROP POLICY IF EXISTS "Users can view own match wins" ON public.match_wins;
DROP POLICY IF EXISTS "Service role can manage match wins" ON public.match_wins;
...
-- Drop existing policies if they exist (IDEMPOTENTE)
DROP POLICY IF EXISTS "Authenticated users can view vault fees" ON public.vault_fees;
DROP POLICY IF EXISTS "Service role can manage vault fees" ON public.vault_fees;
...
-- Drop existing policies if they exist (IDEMPOTENTE)
DROP POLICY IF EXISTS "Authenticated users can view vault balance" ON public.vault_balance;
DROP POLICY IF EXISTS "Service role can manage vault balance" ON public.vault_balance;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'game_config') THEN
        -- Drop existing policies if they exist (IDEMPOTENTE)
        DROP POLICY IF EXISTS "Authenticated users can view game config" ON public.game_config;
        DROP POLICY IF EXISTS "Service role can manage game config" ON public.game_config;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Migración de seguridad completada (IDEMPOTENTE):';
    RAISE NOTICE '   - RLS habilitado en todas las tablas públicas';
    RAISE NOTICE '   - Políticas RLS creadas/actualizadas para cada tabla';
    ...
    RAISE NOTICE '';
    RAISE NOTICE '✅ Esta migración puede ejecutarse múltiples veces sin errores';
END $$;
