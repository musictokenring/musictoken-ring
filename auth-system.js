...
function parseOAuthErrorFromUrl() {
    ...
    if (errorDescription) {
        ...
        if (/Database error saving new user/i.test(decoded)) {
            showToast('Google auth: error guardando nuevo usuario. Revisa trigger/permisos en Supabase.', 'error');
        } else {
            showToast(`Google auth: ${decoded}`, 'error');
        }
